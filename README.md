...
2. Analysis with CRISPResso2 – Validated batch files are processed automatically, generating all required outputs. CRISPResso2 was developed by Kendell Clement and Luca Pinello from the Pinello Lab at Massachusetts General Hospital, USA https://crispresso2.pinellolab.org/submission.
...
| `a`             | Amplicon sequence (DNA) 5′→3′ | A, C, G, T only                 | `ATGCGTACG...`        |
| `an`            | Amplicon name                 | Letters, digits, `_`, `-`       | `amplicon_1`          |
| `g`             | sgRNA sequence (guide) 5′→3′  | A, C, G, T only                 | `GACGTTACGT...`       |
